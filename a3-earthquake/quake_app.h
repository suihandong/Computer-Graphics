...
public:
    ...
    Color color(double mag);
    
    float radious(double mag);
...
private:
    ...
    // true if drawing the global
    bool globe_mode_;
    
    std::vector<Color> color_list;
    std::vector<float> radious_list;
    std::vector<Point3> plane_position_list;
    std::vector<Point3> sphere_position_list;
    float alpha = 0;
    float rotation = 0;
